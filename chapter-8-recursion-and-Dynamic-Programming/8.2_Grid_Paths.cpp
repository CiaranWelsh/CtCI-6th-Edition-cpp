    ...
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            if (i == 0 && j == 0)                            //base case, the number of path to origin is 1
                ...
                ways[i][j] = ways[i][j -
                                     1];                  //if row number is zero, then the path to here can only from left
            else if (j == 0)
                ways[i][j] = ways[i -
                                  1][j];                  //if column number is zero, then the path to here can only from top
            else
                ways[i][j] = ways[i - 1][j] +
                             ways[i][j - 1];   //for a general grid point, the path to here can from left or top
        }
    }
