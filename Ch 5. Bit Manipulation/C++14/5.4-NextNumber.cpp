...
int main() {
    for (uint16_t v : {
            0b11011101111U, 0U, 0b1111111111111111U, 0b0111111111111111U, 0b1111111111111110U}) {
        ...
        if (nextMin > 0)
            std::cout << nextMin << " (" << bits(nextMin) << "b " << countBits(static_cast<unsigned>(nextMin))
                      << " bits)";
        else
            std::cout << "Not existing";
        std::cout << "\nPrev max: ";
        if (prevMax > 0)
            std::cout << prevMax << " (" << bits(prevMax) << "b " << countBits(static_cast<unsigned>(prevMax))
                      << " bits)";
        else
            std::cout << "Not existing";
